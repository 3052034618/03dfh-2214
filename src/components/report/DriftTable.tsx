...
import { AlertTriangle, Play, FileText, Lightbulb, Target } from 'lucide-react';
          ...
          <tbody>
            {driftItems.map((item, idx) => {
              const severityColor = getSeverityColor(
                item.durationMin,
                item.maxTemp,
                item.waybill.tempThreshold
              );
              return (
                <tr
                  key={item.id}
                  className="border-t border-cold-border/50 hover:bg-cold-surface/60 transition-colors"
                >
                  ...
                  <td className="px-3 py-3 text-center">
                    <span className={`
                      ...
                      ${stageColors[item.responsibleStage] || stageColors['其他']}
                    `}>
